...
export const getArticles = (numArticles: number = 5, offset: number = 1, userToken?: string | null) => {
...
export const createArticle = (data: createArticleDataType, method: string) => {
  return async (dispatch: Dispatch) => {
    ...
    try {
      const response = await fetch(`https://blog.kata.academy/api/articles/${data.slug ? data.slug : ''}`, {
        method: method,
        headers: {
          'Content-Type': 'application/json',
