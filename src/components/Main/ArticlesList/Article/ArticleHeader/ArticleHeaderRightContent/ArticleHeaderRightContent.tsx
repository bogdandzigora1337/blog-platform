...
export const ArticleHeaderRightContent: React.FC<IArticleHeaderProps> = ({ item }) => {
  ...
  const authorLabel = isUserArticle ? `${item.author.username} (You)` : item.author.username

  return (
    <div className={cl['article-header__right-content']}>
      <div className={cl['article-header__right-content__container']}>
        <h6 className={cl['article-header__right-content__username-author']}>{authorLabel}</h6>
        <p className={cl['article-header__right-content__created-at']}>{formattedDate}</p>
      </div>
