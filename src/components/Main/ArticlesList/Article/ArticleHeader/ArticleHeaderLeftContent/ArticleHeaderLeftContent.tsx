...
const ArticleHeaderLeftContent: React.FC<IArticleHeaderProps> = ({ item }) => {
  ...
  const renderLikeButton = () => {
    return (
      <span className={cl['article-header__left-content__likes__svg']} onClick={handleLikeClick}>
        {userToken ? item.favorited ? <IconLikesActive /> : <IconLikesNotActive /> : <IconLikesNotActive />}
      </span>
    )
  }
  ...
  return (
    <div className={cl['article-header__left-content']}>
      <div className={cl['article-header__left-content__container']}>
        <Link to={`/articles/${item.slug}`} className={cl['article-header__left-content__title']}>
      ...
      <div className={cl['article-header__left-content__tags']}>{renderTags()}</div>
    </div>
  )
}
