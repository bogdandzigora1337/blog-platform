...
const Main: React.FC = () => {
  const hasError = useSelector((state: HasErrorType) => state.articlesReducer.error)

  const isAuthenticated = useSelector((state: IsAuthenticatedType) => !!state.logToAccountReducer.data)

  const articleErrorNotification = () => (
    <h1 className={cl['main__error-notification']}>
      ⚠️ An error occurred while receiving data from the server! Try refreshing the page
    </h1>
  )
  ...
  return (
    <div className={cl['main']}>
      ...
      <Route path={'/new-article'}>{isAuthenticated ? <ArticlesCreating /> : <Redirect to={'/sing-in'} />}</Route>

      <Route path="/articles/:slug/edit">{isAuthenticated ? <ArticlesCreating /> : <Redirect to={'/sing-in'} />}</Route>
