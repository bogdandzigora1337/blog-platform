...
import { createArticle, currentArticlesPage, getArticles } from '../../../../redux/actions/articleActions'
import InputField from '../../Form/InputField/InputField'
import { UserStateType, ArticlesStateType, ArticleDataType } from '../../../../types/types'
...
export const ArticleCreatingForm: React.FC<IArticleFormProps> = ({
  ...
  const {
    ...
    setValue,
    clearErrors,
  } = useForm<ArticlesStateType['articlesReducer']['data']['articles'][number]>({ mode: 'onChange' })
  ...
  const onSubmit = async (data: ArticlesStateType['articlesReducer']['data']['articles'][number]) => {
    const arrayTags = data.tagList
      ? Array.isArray(data.tagList)
        ? data.tagList.map((elem) => (typeof elem === 'string' ? elem : elem.name))
        : [data.tagList]
      : []
    ...
    dispatch(createArticle(wrapperData, isNewArticlePage ? 'POST' : 'PUT')).then((data: unknown) => {
  ...
  const { fields, append, remove } = useFieldArray<ArticlesStateType['articlesReducer']['data']['articles'][number]>({
    name: 'tagList',
    control,
  })
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <TagInput fields={fields} register={register} remove={remove} errors={errors} append={append} />

      <input className={cl['article-creating__form__send']} type="submit" value={'Send'} disabled={!isValid} />
    </form>
  )
}
