...
export const ProfileEditing: React.FC = () => {
  ...
  const editingError = useSelector((state: UserStateType) => state.logToAccountReducer.editingError)

  const userData = useSelector((state: UserStateType) => state.logToAccountReducer.data)
  ...
  const renderContent = () => {
    if (!userToken) {
      ...
      return <ProfileEditingForm userToken={userToken} userData={userData} editingError={editingError} />
    }
  }
